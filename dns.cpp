...
int resolve_host_over_udp(const std::string & host, std::string & ip) {
	...
	if((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		std::cerr << "Failed to create DNS client socket. Errno: " << std::strerror(errno) << std::endl;
		return -1;
	}
	...
	if(sendto(sock, dns_req.c_str(), dns_req.size(), 0, (const struct sockaddr *) &server_address, server_address_len) < 0) {
	...
	for(;;) {
		...
		if(std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count() >= Settings_perst.builtin_dns_req_timeout) {
			std::cerr << "DNS request timeout" << std::endl;
			close(sock);
			return -1;
		}

		int ret = poll(fds, 1, Settings_perst.builtin_dns_req_timeout - std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count());
		if (ret == -1) {
			...
			if (fds[0].revents & POLLIN) {
				if((bytes_read = recvfrom(sock, &response[0], response.size(), 0, (struct sockaddr *) &server_address, &server_address_len)) <= 0) {
					std::cerr << "Failed to get response from DNS server. Errno: " << std::strerror(errno) << std::endl;
					close(sock);
					return -1;
				}
